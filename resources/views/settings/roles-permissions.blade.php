    ...
    <div class="card shadow-sm mt-4 border-0">
      <div class="card-header bg-white border-0 pb-0">
        <div class="d-flex flex-wrap justify-content-between align-items-start gap-2">
          <div>
            <h5 class="mb-1">@lang('permissions.Users Section Title')</h5>
            <p class="text-muted small mb-0">@lang('permissions.Users Section Description')</p>
          </div>
        </div>
      </div>
      <div class="card-body">
        @if($users->isEmpty())
          <div class="alert alert-secondary mb-0">@lang('users.No users found.')</div>
        @elseif($roles->isEmpty())
          <div class="alert alert-warning mb-0">@lang('permissions.No roles available yet.')</div>
        @else
          <div class="row g-3 align-items-end mb-4">
            <div class="col-lg-6">
              <label for="user-search" class="form-label small fw-semibold text-muted text-uppercase">@lang('users.Search Users')</label>
              <div class="input-group search-input-group">
                <span class="input-group-text bg-body"><i class="bi bi-search"></i></span>
                <input type="search" id="user-search" class="form-control" placeholder="@lang('users.Search Users Placeholder')" aria-label="@lang('users.Search Users')">
                <button type="button" class="btn btn-outline-secondary" id="user-search-clear">
                  <i class="bi bi-x-circle me-1"></i>@lang('users.Clear Search')
                </button>
              </div>
              <div class="form-text">@lang('users.Search Users Help')</div>
            </div>
          </div>
          ...
          <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
              <thead class="table-light">
                <tr>
                  <th style="min-width: 200px;">@lang('users.Name')</th>
                  <th style="min-width: 220px;">@lang('users.Roles')</th>
                  <th class="text-center" style="min-width: 220px;">@lang('users.Actions')</th>
                </tr>
              </thead>
              <tbody>
                @foreach($users as $user)
                  <tr data-user-row="{{ Str::lower($user->name . ' ' . $user->email) }}">
                    <td>
                    ...
                    <td>
                      @if($user->roles->isEmpty())
                        <span class="badge bg-secondary-subtle text-secondary">@lang('users.Without Roles')</span>
                      @else
                        <div class="d-flex flex-wrap gap-2">
                          @foreach($user->roles as $role)
                            <span class="badge bg-primary-subtle text-primary">{{ $roleLabels[$role->name] ?? $role->name }}</span>
                          @endforeach
                        </div>
                      @endif
                    </td>
                    <td>
                      <form method="POST" action="{{ route('users.roles.update', $user) }}" class="d-flex flex-column gap-3">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="d-flex flex-wrap gap-3">
                          @foreach($roles as $role)
                            <div class="form-check">
                              <input class="form-check-input" type="checkbox" name="roles[]" value="{{ $role->name }}" id="user-{{ $user->id }}-role-{{ Str::slug($role->name) }}" @checked($user->roles->contains('name', $role->name))>
                              <label class="form-check-label small" for="user-{{ $user->id }}-role-{{ Str::slug($role->name) }}">
                                {{ $roleLabels[$role->name] ?? $role->name }}
                              </label>
                            </div>
                          @endforeach
                        ...
                        <div class="d-flex gap-2 justify-content-center justify-content-md-start">
                          <button type="submit" class="btn btn-sm btn-primary">
                            <i class="bi bi-save me-1"></i>@lang('users.Save')
                          </button>
                          <a href="{{ route('users.roles.edit', $user) }}" class="btn btn-sm btn-outline-secondary">
                ...
                @endforeach
                <tr id="users-empty-results" class="d-none" data-empty-template="@lang('users.No users match query', ['query' => ':query'])">
                  <td colspan="3">
                    <div class="alert alert-light border mb-0">
                      <i class="bi bi-search me-2"></i>
                      <span data-users-empty-text>@lang('users.No users match your search.')</span>
                    </div>
                  </td>
                </tr>
              </tbody>
...
@push('scripts')
  <script>
    document.addEventListener('DOMContentLoaded', function () {
      ...
      const userSearch = root.querySelector('#user-search');
      const userClear = root.querySelector('#user-search-clear');
      const userRows = root.querySelectorAll('tbody tr[data-user-row]');
      const usersEmpty = root.querySelector('#users-empty-results');
      const usersEmptyText = usersEmpty ? usersEmpty.querySelector('[data-users-empty-text]') : null;
      const usersEmptyTemplate = usersEmpty ? usersEmpty.getAttribute('data-empty-template') : null;

      if (usersEmpty && usersEmptyText) {
        usersEmpty.setAttribute('data-default-text', usersEmptyText.textContent);
      }
      ...
      const updateUserVisibility = () => {
        ...
        userRows.forEach(row => {
          const value = row.dataset.userRow || '';
        ...
        if (usersEmpty && usersEmptyText) {
          usersEmpty.classList.toggle('d-none', visibleRows > 0);
          if (query !== '' && usersEmptyTemplate) {
            usersEmptyText.textContent = usersEmptyTemplate.replace(':query', query);
          } else {
            usersEmptyText.textContent = usersEmpty.getAttribute('data-default-text') || '';
          }
        }
      };

      userSearch?.addEventListener('input', updateUserVisibility);
