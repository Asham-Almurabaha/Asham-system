...
@php
    ...
    $pageTitle = $pageTitle ?? 'قيود المكتب — عملية سريعة';
    $pageHeading = $pageHeading ?? 'قيود المكتب — عملية سريعة';
    $shortcutLinks = $shortcutLinks ?? [];

    $today = now()->toDateString();
    $singleAction = route('ledger.store');
    $splitAction = route('ledger.split.store');
    $redirectRouteName = $redirectRouteName ?? 'ledger.office.shortcuts';
...
{{-- @if (!empty($missingStatuses))
    <div class="alert alert-warning">
        الحالات التالية غير متاحة ضمن فئة المكتب: <strong>{{ implode('، ', $missingStatuses) }}</strong>.
            ...
            <div class="tab-content pt-3" id="investorOperationsContent">
                @foreach ($operations as $key => $operation)
                    ...
                    <div
                        ...
                        @php($allowInvestor = (bool) ($operation['allow_investor'] ?? false))
                        <div class="alert alert-info d-flex justify-content-between align-items-center">
                            <div>
                                ...
                                @endif
                                @if($allowInvestor)
                                    <div class="small text-muted mt-1">حقل المستثمر اختياري للربط مع مستثمر محدد.</div>
                                @endif
                            </div>
                        ...
                        @if (!($operation['enabled'] ?? false))
                            <div class="alert alert-danger mb-0">
                                لا يمكن استخدام هذا النموذج لعدم توفر الحالة المرتبطة به ضمن فئة المكتب.
                            </div>
                            @continue
                        @endif
                        ...
                        <div class="tab-content pt-3" id="investor-operation-{{ $key }}-mode-content">
                            <div
                                ...
                                <form
                                    ...
                                    @if ($allowInvestor)
                                        <div class="col-md-4">
                                            <label class="form-label" for="investor-{{ $key }}-single">المستثمر (اختياري)</label>
                                            <div
                                                ...
                                                <select
                                                    ...
                                                    <option value="" {{ $investorSingleId ? '' : 'selected' }}>اختر المستثمر (اختياري)</option>
                                    ...
                                    @if ($allowInvestor)
                                        <div class="col-12">
                                            <div
                                                class="js-investor-field"
                                                data-liquidity-url="{{ route('ajax.investors.liquidity', ['investor' => '__ID__']) }}">
                                                <label class="form-label" for="investor-{{ $key }}-split">المستثمر (اختياري)</label>
                                                <select
                                                    name="investor_id"
                                                    id="investor-{{ $key }}-split"
                                                    class="form-select js-investor-select">
                                                    <option value="" {{ $investorSplitId ? '' : 'selected' }}>اختر المستثمر (اختياري)</option>
                                                    @foreach ($investors as $investor)
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const OPERATION_STORAGE_KEY = 'officeLedgerShortcuts.activeOperation';
            const MODE_STORAGE_PREFIX = 'officeLedgerShortcuts.activeMode.';
