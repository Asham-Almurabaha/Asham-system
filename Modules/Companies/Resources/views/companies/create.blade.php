...
@section('title', __('companies::companies.Add Company'))

@section('content')
<div class="pagetitle mb-3">
  <h1 class="h3 mb-1">{{ __('companies::companies.Add Company') }}</h1>
  <nav>
    <ol class="breadcrumb">
      <li class="breadcrumb-item"><a href="{{ route('companies.index') }}">{{ __('companies::companies.Companies') }}</a></li>
      <li class="breadcrumb-item active">{{ __('companies::companies.Add Company') }}</li>
    </ol>
  </nav>
</div>
    ...
    <form action="{{ route('companies.store') }}" method="POST" novalidate>
      ...
      <div class="row g-3">
        ...
        <div class="col-12">
          ...
          <textarea name="notes" id="notes" rows="4" class="form-control @error('notes') is-invalid @enderror" placeholder="{{ __('companies::companies.Notes Placeholder') }}">{{ old('notes') }}</textarea>
          @error('notes') <div class="invalid-feedback">{{ $message }}</div> @enderror
        </div>
      </div>

      <div class="d-flex align-items-center gap-2 mt-4">
        <x-button.action type="submit" variant="success">{{ __('companies::companies.Save Company') }}</x-button.action>
