...
<div class="card shadow-sm border-0">
  <div class="card-body p-4">
    <form action="{{ route('company-transactions.store') }}" method="POST" novalidate>
      @csrf

      <div class="row g-3">
        <div class="col-md-4">
          <label for="transaction_date" class="form-label">{{ __('companies::companies.Transaction Date') }} <span class="text-danger">*</span></label>
          <input type="date" name="transaction_date" id="transaction_date" value="{{ old('transaction_date', now()->format('Y-m-d')) }}" class="form-control js-date @error('transaction_date') is-invalid @enderror" required>
          @error('transaction_date') <div class="invalid-feedback">{{ $message }}</div> @enderror
        </div>
        <div class="col-md-4">
          <label for="reference" class="form-label">{{ __('companies::companies.Reference') }}</label>
          <input type="text" name="reference" id="reference" value="{{ old('reference') }}" class="form-control @error('reference') is-invalid @enderror" maxlength="190">
        ...
        <div class="col-md-4">
          ...
          <input type="number" step="0.01" min="0.01" name="total_amount" id="total_amount" value="{{ old('total_amount') }}" class="form-control @error('total_amount') is-invalid @enderror" required>
        ...
        <div class="col-md-4">
          ...
          <select name="bank_account_id" id="bank_account_id" class="form-select @error('bank_account_id') is-invalid @enderror">
            ...
            @foreach($bankAccounts as $bankAccount)
              <option value="{{ $bankAccount->id }}" @selected((int) old('bank_account_id') === $bankAccount->id)>{{ $bankAccount->name }}</option>
            @endforeach
          </select>
        ...
        <div class="col-md-4">
          ...
          <select name="safe_id" id="safe_id" class="form-select @error('safe_id') is-invalid @enderror">
            ...
            @foreach($safes as $safe)
              <option value="{{ $safe->id }}" @selected((int) old('safe_id') === $safe->id)>{{ $safe->name }}</option>
            @endforeach
          </select>
        ...
        <div class="col-12">
          ...
          <textarea name="notes" id="notes" rows="3" class="form-control @error('notes') is-invalid @enderror" placeholder="{{ __('companies::companies.Notes Placeholder') }}">{{ old('notes') }}</textarea>
