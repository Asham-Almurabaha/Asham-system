...
@section('title', __('guarantors::messages.View Guarantor'))
            ...
            $activeContractsCount = $guarantor->contracts()->where(function ($q) use ($endedNames) {
                ...
                // status_id أو contract_status_id
                    ...
                    if (class_exists(\App\Models\Lookups\ContractStatus::class)) {
                        $endedIds = \App\Models\Lookups\ContractStatus::whereIn('name', $endedNames)->pluck('id');
    ...
    <div class="row g-3 mb-3">
        <div class="col-12 col-md-4">
            <div class="kpi-card p-3 h-100">
                ...
                <div class="small text-muted">{{ __('guarantors::messages.All contracts linked to the guarantor') }}</div>
            </div>
        </div>
        <div class="col-12 col-md-4">
            <div class="kpi-card p-3 h-100">
                ...
                <div class="small text-muted">{{ __('guarantors::messages.Ratio:') }} {{ $contractsCount>0 ? number_format($activePct,1) : 0 }}%</div>
        ...
        <div class="col-12 col-md-4">
            <div class="kpi-card p-3 h-100">
                ...
                <div class="small text-muted">{{ __('guarantors::messages.Last Update:') }} {{ optional($guarantor->updated_at)->format('Y-m-d H:i') ?? '—' }}</div>
            </div>
        </div>
    </div>
    ...
    <div class="card shadow-sm mb-3 kpi-card">
        ...
        <div class="card-body pt-2">
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    <div class="row">
                        <div class="col-5 label-col">{{ __('guarantors::messages.Name') }}</div>
                        <div class="col-7 value-col">{{ $guarantor->name }}</div>
                    </div>
                    <div class="row mt-2">
                        <div class="col-5 label-col">{{ __('guarantors::messages.National ID') }}</div>
                        <div class="col-7 value-col">
                            @if($guarantor->national_id)
                                <span>{{ $guarantor->national_id }}</span>
                                <button class="btn btn-light btn-sm ms-1" onclick="copyText('{{ $guarantor->national_id }}')" title="{{ __('guarantors::messages.Copy') }}">
                    ...
                    <div class="row mt-2">
                        <div class="col-5 label-col">{{ __('guarantors::messages.Nationality') }}</div>
                ...
                <div class="col-md-6">
                    ...
                    <div class="row mt-2">
                        <div class="col-5 label-col">{{ __('guarantors::messages.Address') }}</div>
        ...
        <div class="col-lg-6">
            <div class="card shadow-sm h-100 kpi-card">
                ...
                <div class="card-body">
                    ...
                    @else
                        <div class="text-muted">{{ __('guarantors::messages.No ID card image uploaded.') }}</div>
                    @endif
                </div>
            </div>
        </div>
