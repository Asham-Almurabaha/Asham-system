...
<div class="container-xxl py-4">
        ...
        <div>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-2">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">{{ __('Contract Statuses') }}</li>
                </ol>
            </nav>
            <h1 class="h4 mb-0">{{ __('Contract Statuses List') }}</h1>
        </div>
        <div class="ms-auto d-flex flex-wrap gap-2">
            <a href="{{ route('contract_statuses.create') }}" class="btn btn-success">
    ...
    <div class="card border-0 shadow-sm">
        <x-table head-class="table-light">
                ...
                <tr>
                    ...
                    <td class="text-end">
                        <div class="d-inline-flex gap-2">
                            <a href="{{ route('contract_statuses.edit', $status->id) }}" class="btn btn-sm btn-outline-primary">{{ __('Edit') }}</a>
            ...
            @empty
                <tr>
                    <td colspan="3" class="text-center text-muted py-4">{{ __('No contract statuses found.') }}</td>
                </tr>
            @endforelse
        </x-table>
    </div>
