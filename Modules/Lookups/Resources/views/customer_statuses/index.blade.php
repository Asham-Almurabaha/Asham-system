...
<div class="container-xxl py-4">
        ...
        <div>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-2">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">{{ __('Customer Statuses') }}</li>
                </ol>
            </nav>
            <h1 class="h4 mb-0">{{ __('Customer Statuses List') }}</h1>
        </div>
        <div class="ms-auto d-flex flex-wrap gap-2">
            <x-button.action href="{{ route('customer_statuses.create') }}" variant="success">
                <i class="bi bi-plus-lg me-1"></i>{{ __('Add New Customer Status') }}
            </x-button.action>
        </div>
    </div>
    ...
    <div class="card border-0 shadow-sm">
        <x-table head-class="table-light">
                ...
                <tr>
                    ...
                    <td class="text-end">
                        <div class="d-inline-flex gap-2">
                            <x-button.action href="{{ route('customer_statuses.edit', $status->id) }}" variant="primary" :outline="true" size="sm">{{ __('Edit') }}</x-button.action>
            ...
            @empty
                <tr>
                    <td colspan="3" class="text-center text-muted py-4">{{ __('No customer statuses found.') }}</td>
                </tr>
            @endforelse
        </x-table>
    </div>
