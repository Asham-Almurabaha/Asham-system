...
<div class="container-xxl py-4">
        ...
        <div>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-2">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">{{ __('Claimants') }}</li>
                </ol>
            </nav>
            <h1 class="h4 mb-0">{{ __('Claimants List') }}</h1>
        </div>
        <div class="ms-auto d-flex flex-wrap gap-2">
            <x-button.action href="{{ route('claimants.create') }}" variant="success">
    ...
    <div class="card border-0 shadow-sm">
        <x-table head-class="table-light">
                ...
                <tr>
                    ...
                    <td class="text-end">
                        <div class="d-inline-flex gap-2">
                            <x-button.action href="{{ route('claimants.edit', $claimant->id) }}" variant="primary" :outline="true" size="sm">{{ __('Edit') }}</x-button.action>
            ...
            @empty
                <tr>
                    <td colspan="3" class="text-center text-muted py-4">{{ __('No claimants found.') }}</td>
                </tr>
            @endforelse
        </x-table>
    </div>
