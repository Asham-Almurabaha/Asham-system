        ...
        <div>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-2">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">{{ __('Claim Payers') }}</li>
                </ol>
            </nav>
            <h1 class="h4 mb-0">{{ __('Claim Payers List') }}</h1>
        </div>
        <div class="ms-auto d-flex flex-wrap gap-2">
            <x-button.action href="{{ route('claim_payers.create') }}" variant="success">
                <i class="bi bi-plus-lg me-1"></i>{{ __('Add New Claim Payer') }}
            </x-button.action>
        </div>
    </div>
        ...
        <x-table head-class="table-light">
            ...
            @forelse($claimPayers as $claimPayer)
                <tr>
                    ...
                    <td class="fw-semibold text-start">{{ $claimPayer->name }}</td>
                    <td class="text-center">
                        @include('lookups::components.protection-status', ['isProtected' => $claimPayer->is_protected])
                    </td>
                    <td class="text-end">
                        <div class="d-inline-flex gap-2">
                            <x-button.action
                                href="{{ route('claim_payers.edit', $claimPayer->id) }}"
                                variant="primary"
                                :outline="true"
                                size="sm"
                                :disabled="$claimPayer->is_protected"
                            >{{ __('Edit') }}</x-button.action>

                            @if(! $claimPayer->is_protected)
            ...
            @empty
                <tr>
                    <td colspan="4" class="text-center text-muted py-4">{{ __('No claim payers found.') }}</td>
                </tr>
            @endforelse
        </x-table>
    </div>
