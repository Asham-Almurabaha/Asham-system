...
@section('title', __('Add New Claim First Party'))

@section('content')
    <div class="pagetitle">
      <h1>{{ __('Add New Claim First Party') }}</h1>
      <nav>
        <ol class="breadcrumb">
          <li class="breadcrumb-item">{{ __('Settings') }}</li>
          <li class="breadcrumb-item">{{ __('Claim First Parties') }}</li>
          <li class="breadcrumb-item active">{{ __('Add New Claim First Party') }}</li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
    ...
    <div class="col-lg-6">
        <div class="card">
            <div class="card-body p-20">
              <form action="{{ route('claim_first_parties.store') }}" method="POST">
                    @csrf
